...
Все обращения к PostgreSQL выполняются через репозитории; они не развёрнуты на схеме, чтобы сохранить читаемость. Повторяющиеся сервисы в разных процессах — отдельные экземпляры одних и тех же модулей, а не сетевые микросервисы. Загрузка внешних экономических показателей будет отдельной интеграцией; её детали ещё не определены и здесь не показаны.

Сейчас запускаются два процесса: API с планировщиком и ingestion-worker. Планируется третий — classification-worker, который загружает модель один раз и начинает с одного активного задания (`concurrency: 1`). Это не ограничение числа внутренних CPU-потоков модели. Проект остаётся модульным монолитом в одном репозитории.
