...
Backend сервиса медиааналитики: сбор новостей из разных источников, тематический мониторинг и исследование связи новостной повестки с экономическими и финансовыми показателями.

**TypeScript · Node.js · NestJS · PostgreSQL · BullMQ · Redis**

## Назначение

Проект нацелен на количественный анализ тематической структуры новостного потока и её изменений в периоды движения экономических и финансовых показателей. Основной сценарий — выявление тем, доля которых выросла относительно базового периода, с последующим анализом соответствующих публикаций.

Планируемые задачи:

- Формирование корпуса публикаций из нескольких источников с сохранением происхождения и времени публикации.
- Многотематическая классификация и поиск публикаций по темам, источникам и периодам.
- Расчёт временных рядов количества и доли публикаций по темам с учётом полноты сбора и покрытия классификацией.
- Сопоставление тематической динамики с выбранным показателем в заданных временных окнах, включая периоды перед его изменением.

Результат анализа — тематические временные ряды и связанные с ними публикации для мониторинга и проверки исследовательских гипотез. Предмет анализа охватывает общую новостную повестку, а не только экономические материалы. Выводы ограничены составом наблюдаемого корпуса; временные зависимости сами по себе не устанавливают причинность.

## Текущее состояние

**Сейчас работает ingestion-контур:** REST API, планировщик, отдельный worker, обработка частичных сбоев, идемпотентное сохранение статей и история попыток в PostgreSQL. Фоновая классификация, финансовые данные, аналитические endpoint и UI — следующие этапы. Текущая версия предназначена для локального запуска с доверенными источниками.

## Направление развития

Развитие идёт от работающего сбора данных к тематической выдаче, затем к анализу повестки и сопоставлению с показателями.
...
| Классификация | Запланировано | Фоновая обработка статей, сохранение тем и версии классификатора |
...
Группы обозначают границы процессов и инфраструктуры. Стрелки показывают вызовы и обращения к зависимостям, а не только движение данных. Пунктиром отмечены планируемые связи; соответствующие компоненты подписаны «план».

```mermaid
flowchart TD
    subgraph APP["Процесс API и планировщиков"]
        IS["Ingestion scheduler"] --> IP["Ingestion producer"]
        CS["Classification scheduler · план"] -.-> CP["Classification producer · план"]
        API["REST controllers"] --> READ["Сервисы источников и статей"]
        API -.-> ANALYTICS["Тематическая выдача и AnalyticsService · план"]
    end

    subgraph REDIS["Redis — состояние заданий BullMQ"]
        IQ["Очередь ingestion"]
        CQ["Очередь classification · план"]
    end

    subgraph INGEST["Процесс ingestion-worker"]
    ...
    subgraph CLASSIFY["Процесс classification-worker · план"]
        CPROC["Classification processor"] -.-> CSERVICE["ClassificationService"]
        CSERVICE -.-> ADAPTER["Адаптер классификатора"]
        ADAPTER -.-> MODEL["Локальная модель"]
    end

    DB[("PostgreSQL — одна общая база")]
    FEEDS["Внешние RSS-источники"]

    IP -->|"Задания по sourceId"| IQ
    IP -->|"Через SourcesService: включённые источники"| DB
    IQ --> PROC
    RSS -->|"HTTP"| FEEDS
    RUNS --> DB
    SOURCE --> DB
    ...
    CP -. "Выборка необработанных статей" .-> DB
    CP -. "Задания по articleId" .-> CQ
    CQ -.-> CPROC
    CSERVICE -. "Чтение статьи / запись результата" .-> DB
    ANALYTICS -. "Чтение тем, статей и временных рядов" .-> DB
```

Все обращения к PostgreSQL выполняются через репозитории, опущенные на схеме. Повторяющиеся сервисы в разных процессах — отдельные экземпляры общих модулей. Загрузка внешних экономических показателей запланирована как отдельная интеграция.

Сейчас запускаются два процесса: API с планировщиком и ingestion-worker. Планируется третий — classification-worker, который загружает модель один раз и начинает с одного активного задания (`concurrency: 1`). Число вычислительных потоков модели настраивается отдельно.
...
- **Модульный монолит, два процесса.** API обслуживает HTTP-запросы, планировщик ставит задания, worker выполняет сбор через общие бизнес-модули в отдельном Nest application context.
...
- **Репозитории с явным SQL.** Параметризованные запросы и ограничения PostgreSQL определяют правила сохранения данных.
...
- Таблица `ingestion_runs`: отдельная запись на попытку, время начала и завершения, итоговый статус, счётчики и поля `failure_stage`, `failure_reason`, `error_message`.
...
До публичного развёртывания: защита управляющих endpoint, проверка внешних URL против SSRF, CI и документированный процесс запуска.
